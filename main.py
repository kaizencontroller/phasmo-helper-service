...
def _reset_counter_path() -> Path:
    _STATE_DIR.mkdir(parents=True, exist_ok=True)
    return _STATE_DIR / _RESET_COUNTER_FILE
...
<div id="control" class="app hidden">
  ...
  <div class="panel tracker-panel"><div class="head"><strong>Quick Trackers</strong><span class="muted" id="timerSummary">timers / sanity</span></div><div class="body">
    ...
    <div class="tracker-section sanity-section">
      ...
      <div id="vanGoblinBadge" class="van-goblin hidden">🧌 Van Goblin activity detected. Sanity was logged before evidence moved.</div>
    </div>
  </div></div>
  ...
  <div class="panel action-panel"><div class="body row"><button class="orange" id="reset">Reset</button><button class="blue" id="nextRound">Next Round</button><button class="blue" id="copyOverlay">Copy Overlay URL</button></div></div>
  ...
  <div class="panel danger-zone" id="jumpscarePanel"><div class="body">
    ...
    <div id="jumpscareCount" class="danger-count hidden"></div>
  </div></div>
...
<div id="overlay" class="overlay hidden"><section class="ov-card" id="ovCard"><div class="ov-top"><div class="ov-kicker" id="ovKicker">Next Best Test</div><div class="ov-ghosts" id="ovGhosts"></div></div><div class="ov-step" id="ovStep">Loading</div><p class="ov-sub" id="ovSub"></p><div class="ov-bottom"><div class="ov-evidence" id="ovEvidence"></div><div class="ov-notes"><div class="ov-notes-title">Notes / Chat</div><div class="ov-note-text" id="ovNotes">No behaviors or guesses.</div></div></div></section><div id="ovPanicTakeover" class="ov-panic-takeover hidden"><div class="ov-panic-kicker">CHAT EVENT</div><div class="ov-panic-title">PANIC DECLARED</div><div class="ov-panic-sub">Remain calm. Or at least narrate the chaos clearly.</div></div><div id="ovJumpscare" class="ov-jumpscare hidden"><video id="ovJumpscareVideo" preload="auto" playsinline src="/phasmo/jumpscare-video"></video><div class="ov-jumpscare-label">button pressed</div></div></div>
<script>
const MODE='__MODE__';
...
function renderSetup(){
  ...
  if(award){
    ...
    else award.classList.add('hidden');
  }
...
document.getElementById('mode')?.addEventListener('change',e=>postState({evidenceMode:e.target.value}));document.getElementById('changeResponds')?.addEventListener('click',()=>document.getElementById('respondsChoices').classList.toggle('hidden'));document.getElementById('reset')?.addEventListener('click',async()=>{const ok=await postState({reset:true}); if(ok) location.href=`/phasmo/setup?room=${encodeURIComponent(room)}${token?'&token='+encodeURIComponent(token):''}`;});
document.getElementById('nextRound')?.addEventListener('click',async()=>{const ok=await postState({nextRound:true}); if(ok) location.href=`/phasmo/setup?room=${encodeURIComponent(room)}${token?'&token='+encodeURIComponent(token):''}`;});
document.getElementById('copyOverlay')?.addEventListener('click',()=>navigator.clipboard?.writeText(`${location.origin}/phasmo/overlay?room=${encodeURIComponent(room)}`));document.getElementById('behaviorFilter')?.addEventListener('input',renderBehaviors);
