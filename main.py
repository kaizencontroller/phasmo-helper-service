...
function setupOverlay(){
  const phase=Math.floor(Date.now()/5000)%6;
  const guesses=voteSummary('guesses').slice(0,3);
  const votes=voteSummary('votes').slice(0,3);
  const usefulTips=[
    ['Keep The Flow Moving','One person runs evidence. One person watches behavior. Everyone else stops saying “maybe Demon” every 12 seconds.'],
    ['Temperature Tip','Cold weather can make rooms feel suspicious. Trust the thermometer trend, not vibes.'],
    ['Hunt Audio Tip','If the ghost is loud close but quiet far away, think Myling. If it is loud everywhere, think “run better.”'],
    ['Salt Check','Salt is a fast Wraith check. If salt gets stepped in, Wraith is basically off the menu.'],
    ['Spirit Box Tip','If the board says Alone, stop crowding the room like a haunted group project.'],
    ['Orb Reminder','Ghost Orbs love awkward camera angles. Sweep slowly before ruling them out.'],
    ['Cursed Object Sweep','Cursed items have fixed spawns. Check the likely spawn, mark it off, move on.'],
    ['Process Tip','Evidence narrows the list. Behavior verifies the answer. Panic contributes neither.']
  ];
  const badTips=[
    ['Bad Advice','If the ghost starts hunting, loudly explain that you are a content creator and legally protected.'],
    ['Bad Advice','Always split up. Horror movies have proven this creates the most efficient paperwork.'],
    ['Bad Advice','If the ghost responds to Alone, send the least emotionally stable teammate. Science demands sacrifice.'],
    ['Bad Advice','When in doubt, touch the cursed object. The insurance company loves engagement.'],
    ['Bad Advice','If you hear footsteps, stand perfectly still and negotiate. Ghosts respect confident middle management.'],
    ['Bad Advice','A clean van is a sign nobody is trying hard enough.']
  ];
  const tipPool=usefulTips.concat(badTips);
  const tip=tipPool[Math.floor(Date.now()/5000)%tipPool.length];

  const card=document.getElementById('ovCard');
  card?.classList.remove('final');
  card?.classList.add('pregame');
  document.getElementById('ovKicker').textContent='LOADING SCREEN';
  ...
  if(phase===0){
    ...
    document.getElementById('ovEvidence').innerHTML='Lucky guesses are for bragging rights, not decision making.';
    ...
    document.getElementById('ovEvidence').innerHTML='Use !vote when evidence is thin and chat is being asked to help choose.';
    document.getElementById('ovNotes').innerHTML='Commands: !vote, !votes, !guess, !guesses';
  } else if(phase===2){
    ovStep.textContent='COMMANDS';
    document.getElementById('ovSub').textContent='Viewers: !guess for luck • !vote when asked';
    document.getElementById('ovEvidence').innerHTML='Mods can update the run with !ev, !responds, and behavior commands.';
    document.getElementById('ovNotes').innerHTML='Example: !ev orb no • !responds alone';
  } else {
    ovStep.textContent=tip[0].toUpperCase();
    if(ovStep.textContent.length>18) ovStep.classList.add('xsmall');
    else if(ovStep.textContent.length>13) ovStep.classList.add('small');
    document.getElementById('ovSub').textContent=tip[1];
    document.getElementById('ovEvidence').innerHTML=badTips.includes(tip)?'Questionable loading-screen advice. Follow at your own peril.':'Helpful investigation tip. Probably.';
    document.getElementById('ovNotes').innerHTML=(state.map&&state.map!=='unknown')?`${state.map}${state.weather&&state.weather!=='unknown'?' • '+titleCase(state.weather):''}`:'Set up the contract to begin.';
  }
...
function renderOverlay(){
  ...
  document.getElementById('ovCard')?.classList.remove('pregame');
