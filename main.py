...
function renderTrackers(){
  ...
  if(vg){const hasSanity=sanityAverage()!==null; const evidenceMoved=E.some(k=>state.evidence[k]&&state.evidence[k]!=='unknown'); vg.classList.toggle('hidden', !(hasSanity&&!evidenceMoved&&state.setupComplete===true));}
...
function renderOverlay(){
  ...
  const ovHasSanity=sanityAverage()!==null, ovEvidenceMoved=E.some(k=>state.evidence[k]&&state.evidence[k]!=='unknown');
