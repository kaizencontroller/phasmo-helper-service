...
function renderOverlay(){
  ...
  if(timers.length){noteItems.push(...timers.map(t=>`<span class='ov-note-vote'>${t.key==='incense'?'🔥':t.key==='hunt'?'👻':'⏱'} ${titleCase(t.key)} ${fmtTimer(Math.max(0,t.remain))}</span>`))}
