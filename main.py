...
function setupOverlay(){
  ...
  document.getElementById('ovKicker').textContent='Pre-Game Setup';
  if(phase===0){
    document.getElementById('ovStep').textContent='Lucky Guess Board';
    document.getElementById('ovSub').textContent=guesses.length?guesses.map(v=>`${v.ghost} ${v.count}`).join(' • '):'No lucky guesses yet. Type !guess GhostName.';
  } else if(phase===1){
    document.getElementById('ovStep').textContent='Need Chat Input?';
    document.getElementById('ovSub').textContent=votes.length?`Decision votes: ${votes.map(v=>`${v.ghost} ${v.count}`).join(' • ')}`:'Use !vote GhostName when we need help choosing with limited evidence.';
  } else {
    document.getElementById('ovStep').textContent='Chat Commands';
    document.getElementById('ovSub').textContent='Luck: !guess. Decisions: !vote. Info: !votes, !guesses. Mods: !ev, !responds, !b.';
  }
  document.getElementById('ovGhosts').innerHTML='';
  document.getElementById('ovEvidence').innerHTML=E.map(k=>`<span class='ev-dot' title='${EL[k]}'>${{dots:'◌',emf5:'⚡',freezing:'❄',orbs:'◉',writing:'✎',box:'▣',uv:'☝'}[k]}</span>`).join('');
...
function renderOverlay(){
  ...
  const stepText=isFinal?c[0].name:(nx?'Test '+label[nx.ev]:(nb?'Check '+nb.cat:st.name));
  const ovStep=document.getElementById('ovStep');
  ovStep.textContent=stepText;
  ovStep.classList.remove('small','xsmall');
  if(stepText.length>34) ovStep.classList.add('xsmall');
  else if(stepText.length>28) ovStep.classList.add('small');
  document.getElementById('ovSub').textContent=isFinal?`Only ${c[0].name} remains. Verify with behavior before leaving.`:(nx?`${label[nx.ev]} splits ${nx.y}/${nx.n}.`+(nx.ev==='box'?` ${responseLine()}`:''):(nb?`${nb.label}.`:st.text));
  document.getElementById('ovGhosts').innerHTML=c.slice(0,4).map(g=>`<span class='badge'>${g.name}</span>`).join('');
  document.getElementById('ovEvidence').innerHTML=E.map(k=>`<span class='ev-dot ${state.evidence[k]==='yes'?'yes':state.evidence[k]==='no'?'no':''}' title='${label[k]}: ${state.evidence[k]}'>${icon[k]}</span>`).join('');
  let obs=B.filter(b=>(state.behaviors?.[b.id]||'unknown')!=='unknown');
  let votes=voteSummary('votes').slice(0,1); let guesses=voteSummary('guesses').slice(0,1);
  let timers=activeTimers().slice(0,2);
  let bits=[];
  if(timers.length){bits.push(...timers.map(t=>`<span class='ov-note-vote'>${t.key}: ${fmtTimer(Math.max(0,t.remain))}</span>`))}
  ...
  if(guesses.length && bits.length<3){bits.push(...guesses.map(v=>`<span class='ov-note-vote'>Guess ${v.ghost}: ${v.count}</span>`))}
  if(!bits.length)bits.push('Luck: !guess • Decision: !vote');
  const cautions=weatherWarnings();
  if(cautions.length && bits.length<3) bits.push(`<span class='ov-note-vote'>Weather caution</span>`);
