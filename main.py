...
function setupOverlay(){
  const phase=Math.floor(Date.now()/5500)%7;
  ...
  const fieldTips=[
    {
      ...
      note:'Recommended process: evidence first, behavior second, panic never.'
    },
    {
      title:'Temperature Read',
      sub:'Cold weather can make early room reads noisy.',
      body:'Watch the trend and use the thermometer. Do not let Sunrise, Snow, or “vibes” run the investigation.',
      note:'Quality note: trust measured change over first impressions.'
    },
    {
      title:'Salt Check',
      sub:'Fast Wraith elimination.',
      body:'If salt gets disturbed, Wraith is basically off the menu. If it never steps in salt, start asking questions.',
      note:'Simple tests first. Fancy theories later.'
    },
    {
      title:'Spirit Box Protocol',
      sub:'Read the board before blaming the equipment.',
    ...
    {
      title:'Orb Sweep',
      sub:'Ghost Orbs love bad camera angles.',
      body:'Sweep slowly, adjust height, and check from more than one angle before ruling orbs out.',
  ...
  const legacyTips=[
    {
      ...
      sub:'From the journal of Dale “Tripwire” Mullins, Ghost Hunter, 1994–2025.',
      body:'“If the ghost responds to Alone, send the least emotionally stable teammate. They create the cleanest data.”',
      note:'Recovered beside a Spirit Box that was still on.'
    },
    {
      title:'Archived Field Note',
      sub:'From the notes of Marcy Bell, Ghost Hunter, 1994–2025.',
      body:'“When in doubt, touch the cursed object. The insurance company loves engagement.”',
      note:'Cause of death: excessive engagement.'
    },
    {
      title:'Archived Field Note',
      sub:'From the lecture tapes of Coach Harlan, Ghost Hunter, 1994–2025.',
    ...
    {
      title:'Archived Field Note',
      sub:'From the van log of Kevin No-Clip, Ghost Hunter, 1994–2025.',
      body:'“If you hear footsteps, stand still and negotiate. Ghosts respect confident middle management.”',
  ...
  function setTitle(kicker,title,sub,cls){
  ...
  if(phase===0){
    setTitle('CONTRACT BRIEFING','Case File',(state.map&&state.map!=='unknown')?'Review the run setup before entering.':'Select the contract setup to begin.','pregame-brief');
    const pills=[];
    if(state.map&&state.map!=='unknown')pills.push(`<span class='pg-pill good'>${state.map}</span>`);
    if(state.difficulty&&state.difficulty!=='unknown')pills.push(`<span class='pg-pill'>${titleCase(state.difficulty)}</span>`);
    if(state.weather&&state.weather!=='unknown')pills.push(`<span class='pg-pill hot'>${titleCase(state.weather)}</span>`);
    if(state.responds&&state.responds!=='unknown')pills.push(`<span class='pg-pill'>Responds: ${titleCase(state.responds)}</span>`);
    document.getElementById('ovEvidence').innerHTML=`<div class='pg-pillrow'>${pills.length?pills.join(''):"<span class='pg-pill'>Setup in progress</span>"}</div>`;
    document.getElementById('ovNotes').innerHTML='Chat may use !guess while the contract is being set up.';
  } else if(phase===1){
    setTitle('CHAT BOARD','Lucky Guesses','Bragging rights only. Make your call before the evidence ruins the fun.','pregame-chat');
    document.getElementById('ovEvidence').innerHTML=guesses.length?`<div class='pg-pillrow'>${guesses.map(v=>`<span class='pg-pill vote'>${v.ghost}: ${v.count}</span>`).join('')}</div>`:"<div class='pg-cardline'>No guesses yet. Type <strong>!guess GhostName</strong>.</div>";
    document.getElementById('ovNotes').innerHTML='Lucky guesses do not affect the investigation logic.';
  } else if(phase===2){
    setTitle('CHAT BOARD','Decision Vote','Use when evidence is thin and chat is being asked to help choose.','pregame-chat');
    document.getElementById('ovEvidence').innerHTML=votes.length?`<div class='pg-pillrow'>${votes.map(v=>`<span class='pg-pill vote'>${v.ghost}: ${v.count}</span>`).join('')}</div>`:"<div class='pg-cardline'>When asked, vote with <strong>!vote GhostName</strong>.</div>";
    document.getElementById('ovNotes').innerHTML='Decision votes are separate from lucky guesses.';
  } else if(phase===3){
    setTitle('FIELD COMMS','Commands','Useful chat commands for this run.','pregame-comms');
    document.getElementById('ovEvidence').innerHTML="<div class='pg-command'><code>!guess</code><span>lucky prediction</span><code>!vote</code><span>decision input when asked</span><code>!votes</code><span>current decision board</span></div>";
    document.getElementById('ovNotes').innerHTML='Mods can update evidence with !ev and response with !responds.';
  } else if(phase===4 || phase===5){
    const tip=fieldTips[Math.floor(Date.now()/5500)%fieldTips.length];
    setTitle('LOADING TIP',tip.title,tip.sub,'pregame-tip');
    document.getElementById('ovEvidence').innerHTML=`<div class='pg-quote'>${tip.body}</div>`;
    document.getElementById('ovNotes').innerHTML=tip.note;
  } else {
    const tip=legacyTips[Math.floor(Date.now()/5500)%legacyTips.length];
    setTitle('ARCHIVED FIELD NOTE',tip.title,tip.sub,'pregame-legacy');
    document.getElementById('ovEvidence').innerHTML=`<div class='pg-quote'>${tip.body}</div>`;
    document.getElementById('ovNotes').innerHTML=`<span class='pg-source'>${tip.note}</span>`;
  }
