...
function setupOverlay(){
  ...
  document.getElementById('ovEvidence').innerHTML=E.map(k=>`<span class='ev-dot' title='${EL[k]}'><span class='ev-label'>${short[k]}</span><span class='ev-mark'>${icon[k]}</span></span>`).join('');
...
function renderOverlay(){
  ...
  document.getElementById('ovEvidence').innerHTML=E.map(k=>{
    ...
    return `<span class='ev-dot ${v==='yes'?'yes':v==='no'?'no':''}' title='${label[k]}: ${v}'><span class='ev-label'>${short[k]}</span><span class='ev-mark'>${icon[k]}</span></span>`;
  }).join('');
