...
function renderOverlay(){
  ...
  const jumpActive=(state.jumpscareUntil||0)>Date.now();
  if(jumpLayer){
  ...
  let bits=[];
  if(state.panicNote){bits.push(`<span class='ov-note-bad'>⚠ ${state.panicNote}</span>`)}
  ...
  if(ovHasSanity&&!ovEvidenceMoved&&bits.length<2){bits.push(`<span class='ov-note-good'>🧌 Van Goblin activity detected</span>`)}
...
def apply_command(state: Dict[str, Any], command: str, user: str | None = None) -> Tuple[Dict[str, Any], str]:
    ...
    if cmd in {"!panic"}:
        ...
        state["panicNote"] = f"Chat has formally declared panic. Filed by {user_name}."
        return state, "Panic formally declared. This changes no evidence, but it does improve morale."
