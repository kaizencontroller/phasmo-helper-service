...
let state={evidence:{},behaviors:{},votes:{},responds:'unknown',evidenceMode:'3'}; let expanded={}; let evidenceCollapsed=localStorage.getItem('phasmoEvidenceCollapsed')==='true'; let behaviorCollapsed=localStorage.getItem('phasmoBehaviorCollapsed')==='true'; let cursedCollapsed=localStorage.getItem('phasmoCursedCollapsed')==='true'; let topPanelCollapsed=localStorage.getItem('phasmoTopPanelCollapsed')==='true'; let sanitySaveTimer=null; let setupDirty=false; let statePollBlockedUntil=0; let statePollBackoffMs=5000;
...
async function fetchRoomState(targetRoom=room){
  if(Date.now()<statePollBlockedUntil)return null;
  let r;
  ...
  if(r.status===429){
    const retrySeconds=Math.max(1,Number(r.headers.get('Retry-After')||0));
    const waitMs=Math.max(retrySeconds*1000,statePollBackoffMs);
    statePollBlockedUntil=Date.now()+waitMs;
    statePollBackoffMs=Math.min(60000,statePollBackoffMs*2);
  ...
  statePollBlockedUntil=0;
  statePollBackoffMs=5000;
  lastSyncAt=Date.now();
  renderConnectionStatus(true);
  return data;
}
...
renderConnectionStatus(false); loadAppVersion(); loadSiteBanner(); getState().then(()=>maybeShowFeedbackPrompt()); setInterval(pollState, (MODE==='overlay'?2000:(MODE==='control'?3000:5000))); if(MODE==='overlay')setInterval(()=>renderOverlay(),1000); setInterval(()=>renderConnectionStatus(true),5000);
